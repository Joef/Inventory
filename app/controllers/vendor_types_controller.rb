class VendorTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @vendor_type.save
        format.html { redirect_to vendor_types_path, notice: 'Vendor type was successfully created.' }
        format.json { render action: 'show', status: :created, location: @vendor_type }
      else
  ...
  def update
    respond_to do |format|
      if @vendor_type.update(vendor_type_params)
        format.html { redirect_to vendor_types_path, notice: 'Vendor type was successfully updated.' }
